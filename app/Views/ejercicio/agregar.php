    ...
    <div class="row justify-content-center">
        <div class="col-6">
            ...
            <form action="<?=base_url('ejercicios/insertar')?>" method="POST">
                ...
                <input type="text" name="descripcion" id="descripcion" class="form-control" required value="<?=set_value('descripcion')?>" placeholder="Escibre una descripíon para elejercicio">
                ...
                <select name="idEquipo" id="idEquipo" class="form-select" required value="<?=set_value('idEquipo')?>">
                    <option value="" selected disabled>Seleciona una opción</option>
                    <?php foreach($equipo as $key):?>
                    <option value="<?=$key->idEquipo?>"><?=$key->nombre; print" (".$key->estado .")"; ?></option>
                    <?php endforeach?>
                </select>
